import os
os.system("ssh -p 29418 smudbhari@git.eclipse.org  gerrit query --format json   project:mylyn/org.eclipse.mylyn.reviews --patch-sets --current-patch-set --all-approvals --files --comments --commit-message --dependencies  --submit-records >comments_gerrit.txt")
i=0
iteration=1
j=1
with open("comments_gerrit.txt",'r') as ip:
	for line in ip:
		...
		if ('\"moreChanges\":true}' in line):
			...
			cmd= "ssh -p 29418 smudbhari@git.eclipse.org  gerrit query --format json   project:mylyn/org.eclipse.mylyn.reviews --patch-sets --current-patch-set  --files --comments --commit-message --start {} >>comments_gerrit.txt".format(start)
